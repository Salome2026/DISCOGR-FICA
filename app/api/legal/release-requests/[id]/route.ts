...
import { getReleaseRequest, markReleaseRequestReviewed } from "@/lib/db/legalReleaseRequests";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.action !== "marcar_revisado") {
    return NextResponse.json({ error: "Acción inválida." }, { status: 400 });
  }
  const request = await markReleaseRequestReviewed(id, user.email);
