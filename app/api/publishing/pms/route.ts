...
// puntual) — mismo criterio que app/api/cm/pms/route.ts y
// app/api/legal/pms/route.ts: Publishing necesita saber a qué PM ponerle
// un pendiente de cualquier artista.
export async function GET(req: NextRequest) {
  const user = await getSessionUser(req);
  const roles = user?.roles ?? [];
  if (!user?.email || !(roles.includes("editorial") || roles.includes("management") || roles.includes("admin"))) {
