...
function TaskModal({
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/legal/pms/${artist.artistId}/tasks`, {
        method: "POST",
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 20 }} onClick={onClose}>
      <div className="legal-card" onClick={(e) => e.stopPropagation()} style={{ width: 520, maxWidth: "95vw", maxHeight: "90vh", overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ fontWeight: 700, fontSize: 16 }}>Nuevo pendiente para {artist.artistName}</div>

        <div>
          <label className="rlx-field-label" style={{ fontSize: 12.5, color: "var(--text-2)", marginBottom: 6, display: "block", fontWeight: 600 }}>Título</label>
          <input className="legal-search" style={{ width: "100%" }} value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Ej: Falta firmar el contrato" />
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <button type="button" className="legal-btn-ghost" onClick={onClose}>Cancelar</button>
          <button type="button" className="legal-btn-primary" disabled={sending} onClick={handleSubmit}>
...
function ArtistHistory({ artistId }: { artistId: string }) {
  ...
  function load() {
    fetch(`/api/legal/pms/${artistId}/tasks`)
      .then((r) => r.json())
          ...
          {tasks?.map((t) => (
            <div key={t.id} style={{ fontSize: 11.5, borderTop: "1px solid var(--line-soft)", paddingTop: 4 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 6 }}>
                ...
                <span className={`rlr-badge ${t.status === "Resuelto" ? "enviado" : "pendiente"}`} style={{ fontSize: 10 }}>{t.status}</span>
              </div>
...
function LegalPmsInner() {
  ...
  useEffect(() => {
    fetch("/api/legal/pms").then((r) => r.json()).then((d) => (d.error ? setError(d.error) : setPms(d.pms)));
  }, []);
  ...
  return (
    <LegalShell title="Project Managers responsables" subtitle="Poné un pendiente sobre el artista de cualquier PM" backHref="/panel/legal">
          ...
          {filteredPms.map((pm) => (
            <div key={pm.email}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                <div className="legal-artist-avatar" style={{ width: 32, height: 32, fontSize: 13 }}>{pm.name.charAt(0).toUpperCase()}</div>
                <div style={{ fontWeight: 700, fontSize: 15 }}>{pm.name}</div>
                <div style={{ fontSize: 12.5, color: "var(--text-3)" }}>{pm.email}</div>
              </div>
              <div className="legal-pm-grid">
                {pm.artists.map((a) => (
                  <div key={`${pm.email}-${a.artistId}`} className="legal-card" style={{ display: "flex", flexDirection: "column", gap: 8, padding: "1rem" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                      <div style={{ fontWeight: 600, fontSize: 13.5 }}>{a.artistName}</div>
                      {a.role === "collaborator" && <span className="rlr-badge tipo">Compartido</span>}
                    </div>
                    <button
                      type="button"
...
export default function LegalPmsPage() {
  return (
    <RequireRole allow={["legal", "management"]}>
      <LegalPmsInner />
    </RequireRole>
  );
}
