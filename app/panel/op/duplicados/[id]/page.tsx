...
import type { OpDuplicado, OpAuditEntry } from "@discografica/shared/types/op";
...
function Detail({ id }: { id: string }) {
  const [row, setRow] = useState<OpDuplicado | null | undefined>(undefined);
  const [history, setHistory] = useState<OpAuditEntry[] | null>(null);

  useEffect(() => {
    fetch(`/api/op/duplicados/${id}`).then((r) => r.json()).then((d) => setRow(d.row ?? null));
    fetch(`/api/op/duplicados/${id}/history`).then((r) => r.json()).then((d) => setHistory(d.history ?? []));
  }, [id]);
...
export default function DuplicadoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return (
    <RequireRole allow={["op"]}>
      <OpShell title="Ficha de duplicado" backHref="/panel/op/duplicados">
        <Detail id={id} />
      </OpShell>
    </RequireRole>
  );
