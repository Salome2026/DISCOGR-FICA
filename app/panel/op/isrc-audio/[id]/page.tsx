...
import type { OpIsrcAudio, OpAuditEntry } from "@discografica/shared/types/op";
...
function Detail({ id }: { id: string }) {
  const [row, setRow] = useState<OpIsrcAudio | null | undefined>(undefined);
  const [history, setHistory] = useState<OpAuditEntry[] | null>(null);

  useEffect(() => {
    fetch(`/api/op/isrc-audio/${id}`).then((r) => r.json()).then((d) => setRow(d.row ?? null));
    fetch(`/api/op/isrc-audio/${id}/history`).then((r) => r.json()).then((d) => setHistory(d.history ?? []));
  }, [id]);
  ...
  return (
    <>
      ...
      <Section title="Datos">
        ...
        <Row label="Track" value={row.track} />
      </Section>
...
export default function IsrcAudioDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return (
    <RequireRole allow={["op"]}>
      <OpShell title="Ficha ISRC Audio" backHref="/panel/op/isrc-audio">
        <Detail id={id} />
      </OpShell>
    </RequireRole>
  );
