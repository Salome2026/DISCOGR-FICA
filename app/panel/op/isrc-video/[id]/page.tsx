...
import type { OpIsrcVideo, OpAuditEntry } from "@discografica/shared/types/op";
...
function Detail({ id }: { id: string }) {
  const [row, setRow] = useState<OpIsrcVideo | null | undefined>(undefined);
  const [history, setHistory] = useState<OpAuditEntry[] | null>(null);

  useEffect(() => {
    fetch(`/api/op/isrc-video/${id}`).then((r) => r.json()).then((d) => setRow(d.row ?? null));
    fetch(`/api/op/isrc-video/${id}/history`).then((r) => r.json()).then((d) => setHistory(d.history ?? []));
  }, [id]);
  ...
  return (
    <>
      <div className="op-card">
        <div style={{ fontSize: 19, fontWeight: 700 }}>{row.video ?? "—"}</div>
        <div style={{ fontSize: 13, color: "var(--text-3)", marginTop: 2 }}>{row.artista}</div>
      </div>
      <Section title="Datos">
        <Row label="Año" value={row.year} />
        <Row label="ISRC" value={row.isrc} />
        <Row label="Artista" value={row.artista} />
        <Row label="Video" value={row.video} />
      </Section>
...
export default function IsrcVideoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return (
    <RequireRole allow={["op"]}>
      <OpShell title="Ficha ISRC Video" backHref="/panel/op/isrc-video">
        <Detail id={id} />
      </OpShell>
    </RequireRole>
  );
