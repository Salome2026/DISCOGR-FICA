...
import type { OpIsrcVideo, OpIsrcVideoInput } from "@discografica/shared/types/op";

function IsrcVideoForm({ initial, onCancel, onSaved }: { initial: OpIsrcVideo | null; onCancel: () => void; onSaved: () => void }) {
  const [form, setForm] = useState<OpIsrcVideoInput>({ year: initial?.year ?? new Date().getFullYear(), isrc: initial?.isrc ?? "", artista: initial?.artista ?? "", video: initial?.video ?? "" });
  ...
  async function save() {
    ...
    setError(null);
    try {
      const url = initial ? `/api/op/isrc-video/${initial.id}` : "/api/op/isrc-video";
  ...
  return (
    <OpModal onClose={onCancel}>
      <div style={{ fontSize: 16, fontWeight: 700 }}>{initial ? "Editar registro" : "Nuevo ISRC de video"}</div>
      ...
      <div><label className="op-field-label">Video</label><input className="op-input" value={form.video ?? ""} onChange={(e) => setForm({ ...form, video: e.target.value })} /></div>
...
function IsrcVideoContent() {
  const searchParams = useSearchParams();
  const { data: session } = useSession();
  const user = session?.user as unknown as SessionUser | undefined;
  const canEdit = !!user && hasPermission(user, "editar_op");

  const [rows, setRows] = useState<OpIsrcVideo[] | null>(null);
  const [editing, setEditing] = useState<OpIsrcVideo | "new" | null>(null);

  function load() {
    fetch("/api/op/isrc-video").then((r) => r.json()).then((d) => setRows(d.rows ?? []));
  }
  useEffect(load, []);
  ...
  const columns: OpColumn<OpIsrcVideo>[] = [
    { key: "year", label: "Año" }, { key: "isrc", label: "ISRC" }, { key: "artista", label: "Artista" }, { key: "video", label: "Video" },
  ];
  const filters: OpFilter<OpIsrcVideo>[] = [
    { key: "year", label: "Año", options: yearOptions, getValue: (r) => String(r.year) },
  ];

  async function handleDelete(row: OpIsrcVideo) {
    const res = await fetch(`/api/op/isrc-video/${row.id}`, { method: "DELETE" });
    if (res.ok) load();
  }

  if (!rows) return <OpShell title="ISRC Video" backHref="/panel/op"><p style={{ color: "var(--text-3)" }}>Cargando...</p></OpShell>;

  return (
    <OpShell title="ISRC Video" subtitle={`${rows.length} registros`} backHref="/panel/op">
      <OpDataTable
        ...
        detailHref={(row) => `/panel/op/isrc-video/${row.id}`}
        exportFilename="op-isrc-video"
      />
      {editing && <IsrcVideoForm initial={editing === "new" ? null : editing} onCancel={() => setEditing(null)} onSaved={() => { setEditing(null); load(); }} />}
    </OpShell>
  );
}

export default function IsrcVideoPage() {
  return (
