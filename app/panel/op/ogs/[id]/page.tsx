...
import type { OpOgs, OpAuditEntry } from "@discografica/shared/types/op";
...
function OgsDetail({ id }: { id: string }) {
  const [row, setRow] = useState<OpOgs | null | undefined>(undefined);
  const [history, setHistory] = useState<OpAuditEntry[] | null>(null);

  useEffect(() => {
    fetch(`/api/op/ogs/${id}`).then((r) => r.json()).then((d) => setRow(d.row ?? null));
    fetch(`/api/op/ogs/${id}/history`).then((r) => r.json()).then((d) => setHistory(d.history ?? []));
  }, [id]);
    ...
    <>
      <div className="op-card">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 999, background: row.capif === "SI" ? "var(--good-bg)" : "var(--bg-2)", color: row.capif === "SI" ? "var(--good-ink)" : "var(--text-3)" }}>
            CAPIF: {row.capif}
          </span>
        </div>
      </div>
      ...
      <Section title="Datos">
        ...
        <Row label="Year" value={row.year} />
        <Row label="Provider" value={row.provider} />
        <Row label="Sello" value={row.sello} />
      </Section>

      <Section title="Historial de cambios">
        <OpHistorySection history={history} />
      </Section>
    </>
  );
}

export default function OgsDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return (
    <RequireRole allow={["op"]}>
      <OpShell title="Ficha OGS" backHref="/panel/op/ogs">
        <OgsDetail id={id} />
      </OpShell>
    </RequireRole>
  );
